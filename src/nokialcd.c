...
void lcdInit()
{
  ...
  // Initialize PB10-PB14 as outputs
    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE);
    GPIO_InitStructure.GPIO_Pin = B_LCD_PIN_ENABLE | B_LCD_PIN_RESET | B_LCD_PIN_COMMAND | B_LCD_PIN_DATA | B_LCD_PIN_CLOCK;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  ...
  // Reset LCD GPIOB
    GPIO_WriteBit(GPIOB, B_LCD_PIN_RESET, Bit_SET);
    timer_sleep(10);
    //delayms(10);
    GPIO_WriteBit(GPIOB, B_LCD_PIN_RESET, Bit_RESET);
    timer_sleep(10);
    //delayms(10);
    GPIO_WriteBit(GPIOB, B_LCD_PIN_RESET, Bit_SET);
...
{
  if (dataOrCommand)
    GPIO_WriteBit(GPIOB, B_LCD_PIN_COMMAND, Bit_SET);
  else
    GPIO_WriteBit(GPIOB, B_LCD_PIN_COMMAND, Bit_RESET);

  GPIO_WriteBit(GPIOB, B_LCD_PIN_ENABLE, Bit_RESET);

  int i;
  for (i = 0; i < 8; i++)
  {
    GPIO_WriteBit(GPIOB, B_LCD_PIN_CLOCK, Bit_RESET);
    if (value & 0x80)
      GPIO_WriteBit(GPIOB, B_LCD_PIN_DATA, Bit_SET);
    else
      GPIO_WriteBit(GPIOB, B_LCD_PIN_DATA, Bit_RESET);
    value <<= 1;
    GPIO_WriteBit(GPIOB, B_LCD_PIN_CLOCK, Bit_SET);

  }

  GPIO_WriteBit(GPIOB, B_LCD_PIN_ENABLE, Bit_SET);
